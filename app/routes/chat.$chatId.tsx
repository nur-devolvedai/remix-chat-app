...
export async function loader({ request }: any) {
  ...
  return json({ userChatHistoryAndChats, token }); // Send the fetched data to the client
}

const ChatPage = () => {
  const response = JSON.parse(sessionStorage.getItem("response") || "null");
  const [isSidebarOpen, setIsSidebarOpen] = useState<boolean>(false);
  const navigate = useNavigate();
  const toggleSidebar = (): void => {
    setIsSidebarOpen(!isSidebarOpen);
  };
  const userChatHistory = useLoaderData<typeof loader>();
  // useUserToken(userChatHistory.token)
  console.log("User Token-->", userChatHistory.token)
  const userChatHistoryAndChats = userChatHistory.userChatHistoryAndChats;
  ...
  useEffect(() => {
    ...
    if (storedResponse) {
      setchatResponse(JSON.parse(storedResponse));
    }
  }, []);
  ...
  //       // setTypedResponse((prev) => prev + chatResponse[currentIndex]);
  //       setTypedResponse((prev) => prev + chatResponse[currentIndex]);
  //       currentIndex++;
  ...
  //       }
  //     }, 10); // Adjust typing speed (50ms per character)
  //   }
  ...
  useEffect(() => {
    if (chatResponse) {
      console.log("chatResponse", chatResponse);
      let currentIndex = 0;
      let currentTypedResponse = ""; // Local variable to track typed response
      const interval = setInterval(() => {
        currentTypedResponse += chatResponse[currentIndex]; // Append the current character
        setTypedResponse(currentTypedResponse); // Update state with the new value
        currentIndex++;
  
        console.log("typedResponse", currentTypedResponse); // Log the local variable
        if (currentIndex >= chatResponse.length) {
          clearInterval(interval);
        }
      }, 10); // Adjust typing speed (10ms per character)
    }
  }, [chatResponse]);
...
<section className="flex flex-col w-full overflow-y-scroll md:max-h-[80vh]">
       <section className="flex flex-col w-full  lg:overflow-y-scroll md:max-h-[80vh]">
       <div className="flex-1 w-full lg:w-2/5 max-w-screen-xl my-20 mx-auto">
      <h1></h1>
      <div className="flex flex-col w-full overflow-y-auto scrollbar-hidden max-h-[80vh]">
      ...
      {/* <div className="relative max-w-screen-xl mx-auto w-full bg-white lg:w-2/5 bottom-8"> */}
      <div className="w-full fixed bottom-8 left-1/2 lg:w-2/5 transform -translate-x-1/2 p-2">
            <textarea
              value={prompt}
            ...
            <button
              ...
              className="absolute bottom-1 right-1 m-2 mb-6 py-1 px-1 rounded-full flex items-center pointer-events-auto"
            >
              <svg
                width="28"
                height="28"
